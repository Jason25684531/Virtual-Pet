...
class BrainEngine(QThread):
    ...
    def attach_dispatcher(self, dispatcher) -> bool:
        dispatch = getattr(dispatcher, "dispatch", None)
        if not callable(dispatch):
            return False
        self.message_received.connect(dispatch)
        return True
    ...
    def _build_conversation_chain(self, profile: BrainProfile, system_prompt: str, knowledge_context: str):
        ...
        prompt = PromptTemplate(
            input_variables=["history", "input", "system_prompt", "knowledge_context"],
            template=(
                "{system_prompt}\n\n"
                "{knowledge_context}"
                "以下是你與使用者的對話歷史：\n{history}\n\n"
...
def run_brain_engine_normalization_probe() -> dict[str, object]:
    raw_reply = "當然可以，我來幫你整理重點。 [ACTION:report_news]"
    normalized = BrainEngine._normalize_reply(raw_reply)
    return {
        "raw_reply": raw_reply,
        "normalized_reply": normalized,
        "sanitized_tts_text": sanitize_tts_text(normalized),
        "ok": normalized == "當然可以，我來幫你整理重點。 [ACTION:report_news]",
    }
