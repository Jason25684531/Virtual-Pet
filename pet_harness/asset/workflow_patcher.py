...
class WorkflowPatcher:
    ...
    def parse_active_motions(self) -> list[tuple[str, int, str]]:
        inputs = self._input(self._source, "657", "motion_id_1")
        motions: list[tuple[str, int, str]] = []
        names = {"雀躍大笑": "laugh", "薄怒嘟嘴": "angry", "尷尬擺手": "awkward", "無言微翻白眼": "speechless", "專心聆聽": "listen", "愉悅微笑": "idle", "Waving": "wave_response"}
    ...
    def patch_video(self, *, image: str, selector: int, motion_slot: int, seed: int, prefix: str) -> dict[str, Any]:
        ...
        inputs = self._input(workflow, "657", "motion_id_1")
        for slot in range(1, 31):
            key = f"motion_id_{slot}"
            if key not in inputs:
                raise WorkflowPatchError(f"missing input: 657.{key}")
            if slot != motion_slot:
                inputs[key] = 0
        self._input(workflow, "708", "seed")["seed"] = seed
        self._input(workflow, "742", "filename_prefix")["filename_prefix"] = prefix
