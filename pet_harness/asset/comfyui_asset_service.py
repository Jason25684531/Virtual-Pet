...
class ComfyUIAssetService(AssetService):
    def __init__(self, orchestrator: AssetOrchestrator, worker: AssetJobWorker, character_id: str, library: CharacterLibrary | None = None) -> None:
    ...
    def create_asset(self, request: AssetRequest) -> AssetResponse:
        ...
        threading.Thread(target=self.worker.run_once, daemon=True, name="comfyui-asset-worker").start()
    ...
    def create_character_motion_request(self, source_event_id: str) -> AssetResponse:
        ...
        for _child in children:
            threading.Thread(target=self.worker.run_once, daemon=True, name="comfyui-motion-worker").start()
