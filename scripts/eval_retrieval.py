...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--baseline", action="store_true"); parser.add_argument("--compare", action="store_true"); parser.add_argument("--eval-set", default="tests/data/retrieval_eval_set.json")
    args = parser.parse_args(); cases = json.loads(Path(args.eval_set).read_text(encoding="utf-8"))
    store = QdrantMemoryStore(character_id="miku", collection="miku_memory", path="data/characters/miku/qdrant")
    deadline = time.monotonic() + 30
    while store.status().state == "loading" and time.monotonic() < deadline: time.sleep(.1)
    rankings, durations = [], []
    for case in cases:
        started = time.perf_counter(); hits = store.recall(case["current_turn"], top_k=5); durations.append((time.perf_counter()-started)*1000)
        rankings.append([hit.text for hit in hits])
    report = {"baseline": {"store_status":store.status().state, **metrics(rankings, cases, durations)}}
    if args.compare:
        store.shutdown()
        ...
        for case in cases:
            started = time.perf_counter(); result = retriever.retrieve(RetrievalRequest("miku", case["current_turn"], case.get("previous_user"), case.get("previous_assistant"))); durations.append((time.perf_counter()-started)*1000)
            ...
            wrong_assistant_evidence += sum(
                bool(case.get("previous_assistant")) and item.text == case["previous_assistant"] for item in result.evidence
            )
        report["hybrid"] = {"store_status": hybrid.status().state, **metrics(rankings, cases, durations), "cross_character_leakage": cross_character_leakage, "wrong_assistant_evidence": wrong_assistant_evidence}
